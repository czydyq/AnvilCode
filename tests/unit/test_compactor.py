...
from anvil_code.core.llm.types import LlmResponse, UsageStats
...
# 设计：stub provider 返回非空摘要，断言 chat 调用一次，tool_schemas=[]；用 async def 而非
#       get_event_loop().run_until_complete()，因为后者依赖主线程遗留事件循环，
#       在 pytest-asyncio auto 模式下会被别的用例清空，导致与执行顺序耦合的假失败
...
#       长度与 role 顺序同时断言，才能保证压缩结果对下一次 LLM 调用是合法消息序列
...
#       压缩失败必须是无损的，否则一次网络抖动就会让模型丢掉全部历史
