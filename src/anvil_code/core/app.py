...
import anvil_code
from anvil_code.core.bus.commands import (
    AgentRunCommand,
    AgentRunResult,
...
from anvil_code.core.bus.envelope import EventPushEnvelope
from anvil_code.core.config import AnvilConfig, get_config
from anvil_code.core.events.bus import EventBus
from anvil_code.core.llm.provider import AnthropicProvider
from anvil_code.core.logging_setup import setup_logging
from anvil_code.core.mcp.server import McpServerManager
from anvil_code.core.permissions.manager import PermissionManager
from anvil_code.core.permissions.storage import load_policy_file
from anvil_code.core.runner import AgentRunner
from anvil_code.core.runs import events_file, new_run_id
from anvil_code.core.session import SessionManager, SessionStore
from anvil_code.core.trace.record import TraceRecord
from anvil_code.core.trace.writer import TraceWriter
from anvil_code.core.transport.ipc_broadcaster import IpcEventBroadcaster
from anvil_code.core.transport.socket_server import SocketServer, get_connection_writer
...
class CoreApp:
    def __init__(self) -> None:
        ...
        self._config: AnvilConfig | None = None
    ...
    async def _ping_handler(self, params: dict[str, Any]) -> PongResult:
        ...
        return PongResult(
            server_version=anvil_code.__version__,
    ...
    async def _replay_events(
        ...
        if not path.exists():
            for candidate in Path("~/.anvil/sessions").expanduser().glob(
    ...
    async def run(self) -> None:
        ...
        policy_file = Path("~/.anvil/policy.toml").expanduser()
        ...
        sessions_root = Path("~/.anvil/sessions").expanduser()
        ...
        logger.info("anvil-core %s listening addr=%s", anvil_code.__version__, addr)
