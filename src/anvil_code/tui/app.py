...
from anvil_code.core.skills.loader import SkillLoader
from anvil_code.core.transport.socket_client import IpcError, SocketClient
...
class AnvilTuiApp(App[None]):
    """AnvilCode TUI：终端滚屏风格，实时展示 agent 执行过程。"""
    ...
    # ASCII banner 为定宽艺术字，物理行无法再折，故对下列行豁免 E501
    _BANNER = (
        "[bold cyan] █████╗ ███╗   ██╗██╗   ██╗██╗██╗      ██████╗ ██████╗ ██████╗ ███████╗[/bold cyan]\n"  # noqa: E501
        "[bold cyan]██╔══██╗████╗  ██║██║   ██║██║██║     ██╔════╝██╔═══██╗██╔══██╗██╔════╝[/bold cyan]\n"  # noqa: E501
        "[bold cyan]███████║██╔██╗ ██║██║   ██║██║██║     ██║     ██║   ██║██║  ██║█████╗[/bold cyan]\n"  # noqa: E501
    ...
    def compose(self) -> ComposeResult:
        yield Label("[bold]AnvilCode[/bold]  [dim]connecting...[/dim]", id="header")
    ...
    def _update_header(self, state: str) -> None:
        ...
        header.update(
            f"[bold]AnvilCode[/bold]  [dim]{self._host}:{self._port}[/dim]"
    ...
    async def _socket_loop(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                header.update(f"[bold]AnvilCode[/bold]  [red]subscribe error: {e}[/red]")
            finally:
...
# TUI 入口：读取配置并启动 AnvilTuiApp
def run(config: AnvilConfig, replay_run_id: str | None = None) -> None:
    app = AnvilTuiApp(config.host, config.port, replay_run_id=replay_run_id)
    app.run()
