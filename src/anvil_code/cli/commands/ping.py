...
import anvil_code
from anvil_code.core.bus.commands import PongResult
from anvil_code.core.bus.envelope import JsonRpcError, JsonRpcSuccess
from anvil_code.core.config import AnvilConfig


# 同步入口：运行 ping 协程，连接失败时打印错误并退出
def cmd_ping(config: AnvilConfig) -> None:
    try:
...
async def _ping(config: AnvilConfig) -> None:
    t0 = time.monotonic()
    ...
    req = {
        ...
        "id": "cli-1",
        "method": "core.ping",
        "params": {"client": f"cli/{anvil_code.__version__}"},
    }
